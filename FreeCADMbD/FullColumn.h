...
namespace MbD
{
    ...
    template <typename T>
    inline FColsptr<T> FullColumn<T>::plusNumber(T num)
    {
        size_t n = this->size();
    ...
    template <typename T>
    inline FColsptr<T> FullColumn<T>::minusNumber(T num)
    {
        size_t n = this->size();
        auto answer = std::make_shared<FullColumn<T>>(n);
        for (size_t i = 0; i < n; i++)
