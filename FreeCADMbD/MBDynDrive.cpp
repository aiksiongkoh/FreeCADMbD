...
std::shared_ptr<MBDynDrive> MBDynDrive::newDrive(std::string statement)
{
    if (lineHasTokens(statement, "cosine")) {
        return MBDynDriveCosine::With();
    }
    if (lineHasTokens(statement, "element")) {
        return MBDynDriveElement::With();
    }
    if (lineHasTokens(statement, "meter")) {
        return MBDynDriveMeter::With();
    }
    if (lineHasTokens(statement, "ramp")) {
        return MBDynDriveRamp::With();
    }
    if (lineHasTokens(statement, "sine")) {
        return MBDynDriveSine::With();
    }
    if (lineHasTokens(statement, "string")) {
...
void MBDynDrive::parseMBDyn(std::string line)
{
    ...
    //iss >> name;
    //arguments.erase(arguments.begin());
    //readStringNoSpacesOffTopEqualOrThrow(arguments, "name");
...
void MBDynDrive::readFunction(std::vector<std::string>& args)
{
...
void MBDynDrive::readDriveElement(std::vector<std::string>& args)
{
