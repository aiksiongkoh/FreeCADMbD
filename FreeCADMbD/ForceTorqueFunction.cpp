...
void MbD::ForceTorqueFunction::useUniqueDispIeJeO()
{
    for (const auto kineIJ : *kineIJs)
    {
        kineIJ->useUniqueDispIeJeO();
    }
    for (const auto kinedotIJ : *kinedotIJs)
    {
        kinedotIJ->useUniqueDispIeJeO();
    }
}

void MbD::ForceTorqueFunction::useUniqueDispIeJeKe()
{
    for (const auto kineIJ : *kineIJs)
    {
        kineIJ->useUniqueDispIeJeKe();
    }
    for (const auto kinedotIJ : *kinedotIJs)
    {
        kinedotIJ->useUniqueDispIeJeKe();
    }
}
...
void ForceTorqueFunction::setformula(Symsptr function)
{
    ...
    for (const auto kineIJ : *kineIJs)
        kineIJ->mbdObject->owner = this;
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->mbdObject->owner = this;
}

void ForceTorqueFunction::postDynPredictor()
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postDynPredictor();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postDynPredictor();
    for (const auto jointAction : *jointActions)
        jointAction->postDynPredictor();
...
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postStaticIteration();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postStaticIteration();
    for (const auto jointAction : *jointActions)
        jointAction->postStaticIteration();
...
void ForceTorqueFunction::preDynOutput()
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->preDynOutput();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->preDynOutput();
    for (const auto jointAction : *jointActions)
        jointAction->preDynOutput();
...
FColDsptr ForceTorqueFunction::pFTpkineIJs()
{
    ...
    std::transform(pFTpkineIJFuncs->begin(),
                   ...
                   pFTpkineIJs->begin(),
                   [&](auto pFTpkineIJFunc)
                   { return pFTpkineIJFunc->getValue(); });
    return pFTpkineIJs;
}
...
FColDsptr ForceTorqueFunction::pFTpkinedotIJs()
{
    ...
    std::transform(pFTpkinedotIJFuncs->begin(),
                   ...
                   [&](auto pFTpkinedotIJFunc)
                   { return pFTpkinedotIJFunc->getValue(); });
...
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->initializeGlobally();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->initializeGlobally();
    for (const auto jointAction : *jointActions)
        jointAction->initializeGlobally();
    pFTpkineIJFuncs = std::make_shared<std::vector<Symsptr>>(kineIJs->size());
    std::transform(kineIJs->begin(),
                   kineIJs->end(),
                   pFTpkineIJFuncs->begin(),
                   [&](auto kineIJ)
                   { return forTorFunc->differentiateWRT(kineIJ)->simplified(); });
...
void ForceTorqueFunction::initializeLocally()
{
    ...
    for (const auto kineIJ : *kineIJs)
        kineIJ->initializeLocally();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->initializeLocally();
    for (const auto jointAction : *jointActions)
        jointAction->initializeLocally();
}

void ForceTorqueFunction::postAccICIteration()
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postAccICIteration();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postAccICIteration();
    for (const auto jointAction : *jointActions)
        jointAction->postAccICIteration();
...
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postCollisionCorrectorIteration();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postCollisionCorrectorIteration();
    for (const auto jointAction : *jointActions)
        jointAction->postCollisionCorrectorIteration();
...
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postCollisionPredictor();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postCollisionPredictor();
    for (const auto jointAction : *jointActions)
        jointAction->postCollisionPredictor();
...
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postDynCorrectorIteration();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postDynCorrectorIteration();
    for (const auto jointAction : *jointActions)
        jointAction->postDynCorrectorIteration();
...
{
    for (const auto kineIJ : *kineIJs)
        kineIJ->postDynOutput();
    for (const auto kinedotIJ : *kinedotIJs)
        kinedotIJ->postDynOutput();
    for (const auto jointAction : *jointActions)
        jointAction->postDynOutput();
