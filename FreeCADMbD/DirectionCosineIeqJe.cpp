/***************************************************************************
 *   Copyright (c) 2023 Ondsel, Inc.                                       *
 *                                                                         *
 *   This file is part of OndselSolver.                                    *
 *                                                                         *
 *   See LICENSE file for details about copyright.                         *
 ***************************************************************************/
 
#include "DirectionCosineIeqJe.h"
#include "EndFrameq.h"
#include "EndFramet.h"

using namespace MbD;

std::shared_ptr<DirectionCosineIeqJe> DirectionCosineIeqJe::With(EndFrmsptr frmi, EndFrmsptr frmj, size_t axisi, size_t axisj)
{
    if (std::dynamic_pointer_cast<EndFramet>(frmi)) {
        throw SimulationStoppingError("To be implemented.");
    }
    assert(frmi->has_qX());
    assert(!frmj->has_qX());
    auto inst = std::make_shared<DirectionCosineIeqJe>(frmi, frmj, axisi, axisj);
    inst->initialize();
    return inst;
}

void DirectionCosineIeqJe::initialize()
{
    DirectionCosineIeJe::initialize();
    pAijIeJepEI = FullRow<double>::With(4);
    ppAijIeJepEIpEI = FullMatrix<double>::With(4, 4);
}

void DirectionCosineIeqJe::initializeGlobally()
{
    ppAjOIepEIpEI = std::static_pointer_cast<EndFrameq>(eFrmI)->ppAjOepEpE(axisI);
}

FMatDsptr DirectionCosineIeqJe::ppvaluepEIpEI()
{
    return ppAijIeJepEIpEI;
}

FRowDsptr DirectionCosineIeqJe::pvaluepEI()
{
    return pAijIeJepEI;
}

void DirectionCosineIeqJe::simUpdateAll()
{
    //cos(the) = aAijIeJe = aAcoliOIe->dot(aAcoljOJe);
    DirectionCosineIeJe::simUpdateAll();
    pAjOIepEIT = std::static_pointer_cast<EndFrameq>(eFrmI)->pAjOepET(axisI);
    for (size_t i = 0; i < 4; i++)
    {
        pAijIeJepEI->at(i) = pAjOIepEIT->at(i)->dot(aAjOJe);
    }
    for (size_t i = 0; i < 4; i++)
    {
        auto ppAijIeJepEIipEI = ppAijIeJepEIpEI->at(i);
        auto ppAjOIepEIipEI = ppAjOIepEIpEI->at(i);
        for (size_t j = 0; j < 4; j++)
        {
            ppAijIeJepEIipEI->at(j) = ppAjOIepEIipEI->at(j)->dot(aAjOJe);
        }
    }    
    ppAijIeJepEIpEI->symLowerWithUpper();
}
