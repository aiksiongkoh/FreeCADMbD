...
void ConstraintSet::postStaticIteration()
{
    constraintsDo([&](std::shared_ptr<Constraint> con)
                  { con->postStaticIteration(); });
}

void ConstraintSet::preStatic()
{
    constraintsDo([&](std::shared_ptr<Constraint> con)
                  { con->preStatic(); });
}
...
void MbD::ConstraintSet::fillStaticError(FColDsptr col)
{
    constraintsDo([&](std::shared_ptr<Constraint> con)
                  { con->fillStaticError(col); });
}

void MbD::ConstraintSet::fillStaticJacob(SpMatDsptr mat)
{
    constraintsDo([&](std::shared_ptr<Constraint> con)
