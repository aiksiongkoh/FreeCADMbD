...
std::string ASMTItemIJ::reportComparisonWith(std::shared_ptr<ASMTItem> otherItem)
{
    ...
    auto other = std::dynamic_pointer_cast<ASMTItemIJ>(otherItem);
    if (!other)
    {
        return fullName("") + " comparison item is not an ASMTItemIJ.\n";
    }

    auto markerReport = [this](const std::string &label,
                               const std::shared_ptr<ASMTMarker> &marker,
                               const std::shared_ptr<ASMTMarker> &otherMarker)
    {
        ...
        return marker->reportComparisonWith(otherMarker);
    };

    report = markerReport("markerI", markerI, other->markerI);
    if (!report.empty())
    {
        return report;
    }
    report = markerReport("markerJ", markerJ, other->markerJ);
    if (!report.empty())
    {
        return report;
    }
