...
void GeneralSpline::initxdegreexsys(Symsptr arg, size_t order, std::shared_ptr<std::vector<double>> xarr, std::shared_ptr<std::vector<double>> yarr)
{
    xx = arg;
    degree = order;
    xs = xarr;
...
void GeneralSpline::computeDerivatives()
{
...
bool GeneralSpline::isCyclic() const
{
    return (ys->size() > 3) && (ys->front() == ys->back());
}

double GeneralSpline::derivativeAt(size_t n, double xxx)
{
...
void GeneralSpline::calcIndexAndDeltaFor(double xxx)
{
    xvalue = xxx;
    if (isCyclic()) {
...
void GeneralSpline::calcCyclicIndexAndDelta()
{
...
void GeneralSpline::calcNonCyclicIndexAndDelta()
{
    double xFirst = xs->front();
    double xLast = xs->back();
    if (xvalue <= xFirst) {
...
void GeneralSpline::calcIndexAndDelta()
{
...
void GeneralSpline::searchIndexFromto(size_t first, size_t last)
{
...
Symsptr GeneralSpline::clonesptr()
{
